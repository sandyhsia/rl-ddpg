...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        s_W_fc2 =  weight_variable([64, 20])
        s_b_fc2 = bias_variable([20])
        a_W_fc2 =  weight_variable([self.a_dim, 20])
        a_b_fc2 = bias_variable([20])

        W_critic_Q = weight_variable([20, 1])
        b_critic_Q = bias_variable([1])

        c_h1_layer = tf.nn.relu(tf.matmul(inputs, s_W_fc1) + s_b_fc1)
        c_h2_layer = tf.nn.relu(tf.matmul(c_h1_layer, s_W_fc2) + s_b_fc2 + tf.matmul(action, a_W_fc2) + a_b_fc2)
        
        out = tf.matmul(c_h2_layer, W_critic_Q) + b_critic_Q
