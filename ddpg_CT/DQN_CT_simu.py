...
class ActorNetwork(object):
    ...
    def create_actor_network(self): 
        ...
        a_h1_layer = tf.nn.relu(tf.matmul(inputs, self.s_W_fc1) + self.s_b_fc1)
        a_h2_layer = tf.nn.relu(tf.matmul(a_h1_layer, self.s_W_fc2) + self.s_b_fc2)
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        c_h1_layer = tf.nn.relu(tf.matmul(inputs, self.s_W_fc1) + self.s_b_fc1)
        c_h2_layer = tf.nn.relu(tf.matmul(c_h1_layer, self.s_W_fc2) + self.s_b_fc2 + tf.matmul(action, self.a_W_fc2) + self.a_b_fc2)
...
class SimulatorNetwork(object):
    ...
    def create_network(self):
            ...
            self.state_output = tf.matmul(h1_layer_concat, self.W3) + self.b3
    ...
    def predict_next(self, state, action):
                ...
                state_output = self.state_output.eval(feed_dict={
...
class DQN_CT():
        ...
        def __init__(self, env):
                ...
                #saver = tf.train.Saver(tf.trainable_variables()[0:(len(self.critic.network_params)*2 + self.actor.get_num_trainable_vars())])
                #saver.restore(self.session, "./checkpoint/Use_acff/checkpoint-1878")
        ...
        def train_Q_network(self):
                ...
                if check_next_state == 1:
                        predict_next_batch = self.simulator.predict_next(state_batch, action_batch)
                        print "predict", predict_next_batch[0]
                        print "rt", next_state_batch[0]
                        time.sleep(0.5)
