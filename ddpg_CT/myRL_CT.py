    ...
    try:
            for episode in range(EPISODE):
                    ...
                    for step in range(STEP):
                            ...
                            agent.simulator.perceive(state, action, next_state)
                            #print "rt_next", next_state[330:]
                            #print "predict", agent.simulator.predict_next(np.reshape(state, (-1, 362)), np.reshape(action, (-1, 2)))[0][330:]
                            #time.sleep(0.1)
                            state = next_state
                            # print "reward", reward
                            if done == True:
                                    print "simu_loss", agent.get_simu_loss()
            ...
            if save_request == 1:
                    ...
                    if input == 'y':
                            ...
                            with open(wall_t_fname, 'w') as f:
                                    f.write(time.ctime())
                                    f.write(str(wall_t))
                            agent.all_saver.save(agent.session, CHECKPOINT_DIR + '/' + 'checkpoint', global_step =global_t+episode)
                            print "Save done."
