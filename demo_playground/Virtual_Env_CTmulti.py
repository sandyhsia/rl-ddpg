...
class Virtual_Env():
        ...
        def reset(self, mode, agent_index):
                if mode == 0: # all init
                        ...
                        self.car_center = [[15., 25.], [45., 75.], [75, 35]]
                        self.car_target_pt = list()
                        self.route_pt_counter = list()
                        self.angle = list()
                        self.eat_up_time = list()
                        for i in range(car_agent_num):
                                self.car_target_pt.append(self.driving_route[i*2])
                                self.route_pt_counter.append(i)
                                self.angle.append(270)
                                self.eat_up_time.append(0)
                        ...
                        state, reward, done = self.step_multimode(self.car_center, self.angle, action_array)
                        return state
                         
                elif mode == 1: # init a new target pt
                        self.car_target_pt[agent_index] = self.driving_route[self.route_pt_counter[agent_index]+1]
                        self.route_pt_counter[agent_index] = (self.route_pt_counter[agent_index]+int(random.random()*4))%4
        ...
        def step(self, car_center, angle, action, index):
                ...
                if self._with_cam == 0:
                        # print "action", action
                        car_center, angle, done = self.agent_action(car_center, angle, action, index)
                        # print car_center, angle, done
                        self.car_center[index] = car_center
                ...
                if self._with_cam == 0 and done == True:
                        ...
                        if car_agent_num > 1:
                                ...
                                for i in range(car_agent_num):
                                        if i != index and two_point_distance(self.car_center[index], self.car_center[i]) <= 50:
                                                line_segament = list()
                                                distance_arr = np.empty(0)
                                                points = self.get_head_tail(self.car_center[i], self.angle[i], action, 1)
                                                # print points
                                                for k in range(4):
                                                        distance_arr = np.append(distance_arr, two_point_distance(points[k], self.car_center[index]))

                                                # print distance_arr
                                                
                                                argmin_1 = distance_arr.argmin()
                                                line_segament.append(points[argmin_1])
                                                distance_arr = np.delete(distance_arr, argmin_1)
                                                points = np.delete(points, argmin_1, 0)
                                                # print "line_seg", line_segament
                                                # print "dis_arr", distance_arr
                                                # print "points", points

                                                # points = np.reshape(points, (3,2))
                                                # print "points", points

                                                argmin_2 = distance_arr.argmin()
                                                # print argmin_2
                                                line_segament.append(points[argmin_2])
                                                # print "line_seg", line_segament

                                                distance_vec, intersect_points_vec = self.solve_distance_vec(distance_vec, intersect_points_vec, center, line_segament, len(bounding_lines))
                        ...
                        next_distance_vec = np.append(next_distance_vec, two_point_distance(center, self.car_target_pt[index]))
                        
                        target_direction = vector_direction(center, self.car_target_pt[index])
                        ...
                        if two_point_distance(head, self.car_target_pt[index]) <= 10 or two_point_distance(center, self.car_target_pt[index]) <= 10:
                                reward = 20
                                print "Agent: Yoho!!"
                                self.eat_up_time[index] += 1
                                self.reset(1, index)
                                next_distance_vec, _, _= self.step(car_center, angle, action, index)
                                # print next_distance_vec
                                done = False

                                if DEBUG_MODE == 1:
                                        print "next_distance_vec[0:10]",  next_distance_vec[0:10]
                                        print "target direction", vector_direction(center, self.car_target_pt[index]), "angle", angle
        ...
        def agent_action(self, car_center, angle, action, index):
                ...
                self.display_agent(center, angle_in_rad, head, tail, 1, self.car_target_pt[index], 1)
                ...
                for i in range(car_agent_num):
                        if i != index and two_point_distance(self.car_center[index], self.car_center[i]) <= car_body_lenth*2:
                                done = True
                                print "Other car! Boom!"
        ...
        def display_agent(self, center, angle_in_rad, head, tail, agent_mode, car_target_pt, target_pt_mode):
                ...
                if agent_mode == 0: # snake shape
                        ...
                        if target_pt_mode == 1: # draw
                                pygame.draw.circle(self.screen, [255,0,0], (int(car_target_pt[0]),int(car_target_pt[1])), 2)
        ...
        def solve_distance_vec(self, distance_vec, intersect_points_vec, center, line_segament, j):
                ...
                if j <= len(self.bounding_lines):
                        ...
                        if j == len(self.bounding_lines):
                                bounding_lines = list()
                                if j == len(self.bounding_lines):
                                        if line_segament[0][1] == line_segament[1][1]:
                                                bounding_lines.append([0., 1., -line_segament[0][1]])

                                        elif line_segament[0][0] == line_segament[1][0]:
                                                bounding_lines.append([1., 0., -line_segament[0][0]])

                                        else:
                                                arr1 = [[line_segament[0][1], 1],
                                                [line_segament[1][1], 1]]
                                                arr2 = [-line_segament[0][0], -line_segament[1][0]]
                                                border_line = np.linalg.solve(arr1, arr2)
                                                bounding_lines.append([1., border_line[0], border_line[1]])
        ...
        def step_multimode(self, car_center, angle, action):
                
                self.screen.fill(color)
                
                next_distance_vec_array = list()
                reward_array = list()
                done_array = list()

                for i in range(car_agent_num):
                                
                        next_distance_vec, reward, done = self.step(car_center[i], angle[i], action[i], i)
                        if len(next_distance_vec) == 1:
                                next_distance_vec = np.reshape(next_distance_vec, (362))
                        next_distance_vec_array.append(next_distance_vec)
                        reward_array.append(reward)
                        done_array.append(done)

                return next_distance_vec_array, reward_array, done_array

        def get_head_tail(self, car_center, angle, action, display_mode):
                ...
                if display_mode == 0: # snalke
                        return_arr = np.zeros((2,2))
                        return_arr[0] = head
                        return_arr[1] = tail

                        return return_arr

                elif display_mode == 1: # car
                        head_right = [0, 0]
                        head_left = [0, 0]
                        tail_right = [0, 0]
                        tail_left = [0, 0]
                        ...
                        return_arr = np.zeros((4,2))
                        return_arr[0] = head_left
                        return_arr[1] = head_right
                        return_arr[2] = tail_left
                        return_arr[3] = tail_right
